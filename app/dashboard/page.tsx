    ...
    <div className="min-h-screen" style={{ background: "#F4F6FB" }}>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-[#111111]">Moje galerije</h1>
            <p className="text-sm text-[#5A6068] mt-1">
        ...
        {userAlbums.length === 0 && !dbError ? (
          <div className="text-center py-24 bg-white border border-dashed rounded-2xl" style={{ borderColor: "rgba(255,201,77,0.3)" }}>
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5" style={{ background: "rgba(255,201,77,0.1)" }}>
              <svg className="w-9 h-9" style={{ color: "#8F6900" }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
              </svg>
            </div>
            <h2 className="text-2xl font-extrabold tracking-tight text-[#111111] mb-2">Ustvarite prvo galerijo</h2>
            <p className="text-sm text-[#5A6068] mb-2 max-w-xs mx-auto">
            ...
            <p className="text-xs text-[#5A6068] mt-4">Brezplačno · Do 20 fotografij · QR koda vključena</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {userAlbums.map((album) => (
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-3 left-3">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${
                      ...
                      album.plan === "plus"    ? "text-white bg-[#111111]" :
                      album.plan === "basic"   ? "text-[#111111] bg-[#F4B400]" :
                      "bg-white/80 text-[#5A6068]"
                    }`}
                    style={album.plan === "premium" ? { background: "#F4B400" } : undefined}>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <h3 className="text-lg font-bold text-[#111111] mb-0.5">{album.coupleName}</h3>
                  <p className="text-xs text-[#5A6068] mb-3">
                    {album.weddingDate}{album.location ? ` · ${album.location}` : ""}
                  </p>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex items-center gap-1.5">
                      <svg className="w-3.5 h-3.5" style={{ color: "#8F6900" }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5z" />
                      </svg>
                      <span className="text-xs text-[#5A6068]">{album.photoCount} fotografij</span>
                    </div>
                    ...
                    <span className="ml-auto text-xs text-[#5A6068]">{album.photoCount}/{album.maxPhotos}</span>
                  </div>
                  {/* Action buttons */}
            ...
            <Link
              href="/dashboard/new"
              className="group flex flex-col items-center justify-center h-full min-h-[200px] bg-white rounded-2xl transition-all duration-200 hover:border-[#8F6900]/60"
            ...
            >
              <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-all group-hover:scale-110" style={{ background: "rgba(255,201,77,0.1)" }}>
                <svg className="w-5 h-5" style={{ color: "#8F6900" }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
              </div>
              <span className="text-sm font-semibold" style={{ color: "#8F6900" }}>Nova galerija</span>
            </Link>
          </div>
        )}
      </main>
