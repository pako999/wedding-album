import { HomeWeddingHero } from "@/components/wedding/HomeWeddingHero";
...
export async function LocalizedGuestcamHomePageV3({ lang }: { lang: Lang }) {
  ...
  const createPath = localizedAccountPath(lang, "/dashboard/new");
  const navLinks = ["#how", "#events", "#templates", "#wall", "#pricing", "#business", blogPath] as const;

  const menuLinks = [...t.nav.map((label, i) => ({href: navLinks[i],label})), {href: localePublicPath(lang,WEDDING_PATHS[lang]),label: weddingCopy(lang).nav}];

  return <main className="min-h-screen overflow-hidden bg-[#FFFDF8] text-[#111111]">
    <header className="sticky top-0 z-50 border-b border-black/10 bg-[#FFFDF8]/95 backdrop-blur-xl"><nav className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-3 px-5 sm:px-7 lg:h-[80px] lg:px-8"><Link href={homePath} className="shrink-0"><GuestcamLogo size="md" showMark /></Link><div className="mx-auto hidden items-center gap-4 2xl:flex xl:gap-6">{menuLinks.map(({label,href}) => <Link key={label} href={href} className="group relative whitespace-nowrap py-2 text-[14px] font-bold text-black/65 hover:text-black xl:text-[15px]">{label}<span className="absolute inset-x-0 -bottom-0.5 h-[2.5px] origin-left scale-x-0 rounded-full bg-[#F4B400] transition-transform group-hover:scale-x-100" /></Link>)}</div><div className="ml-auto hidden shrink-0 items-center gap-2 2xl:flex"><LanguageSwitcher current={lang} languages={HOME_HREFLANG} ariaLabel={t.language} /><HeaderAuthButtons lang={lang} signedIn={signedIn} />{!signedIn ? <Link href={createPath} className="rounded-full bg-black px-5 py-3 text-sm font-black text-white">{t.create}</Link> : null}</div><div className="ml-auto 2xl:hidden"><HomeMobileMenu signedIn={signedIn} lang={lang} links={menuLinks} labels={{ open: t.openMenu, close: t.closeMenu, language: t.language, languageAria: t.language, signIn: t.signIn, dashboard: t.dashboard, cta: t.create }} /></div></nav></header>

    <HomeWeddingHero lang={lang} title={t.heroTitle} accent={t.heroAccent} lead={t.heroLead} start={t.start} note={t.note}/>
    ...
    <section className="border-y border-black/10 bg-[#FFF4B8] py-24 text-center sm:py-28"><div className="mx-auto max-w-4xl px-5 sm:px-8"><p className="text-xs font-black uppercase tracking-[.18em] text-black/45">{t.finalEyebrow}</p><h2 className="mt-4 text-4xl font-black leading-[1.04] tracking-[-.05em] sm:text-6xl">{t.finalTitle}</h2><p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-black/55">{t.finalLead}</p><Link href={createPath} className="mt-9 inline-flex rounded-full bg-black px-9 py-4 font-black text-white">{t.finalCta}</Link></div></section>
    <SeoFooter lang={lang} />
  </main>;
}
