...
async function Header() {
  ...
  const nav = [["#how","Kako deluje"],["/porocni-paket","Poročni paket"],["#events","Dogodki"],["#templates","Predloge"],["#wall","Live Wall"],["#pricing","Cenik"],["#business","Za podjetja"],["/blog","Blog"]] as const;
...
export async function GuestcamHomePage() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#FFFDF8] text-[#111111]">
    ...
    <HomeWeddingHero lang="sl" title="Vsi spomini vaših gostov." accent="Na enem mestu." lead="Z eno QR kodo zberite fotografije in videe gostov v zasebni galeriji. Brez aplikacije in brez registracije za goste." start="Začnite brezplačno" note="Brez kreditne kartice · Enkratno plačilo, brez naročnine" />
    ...
    <section id="pricing" className="py-24 sm:py-32"><div className="mx-auto max-w-[1320px] px-5 sm:px-8"><div className="mx-auto max-w-3xl text-center"><SectionTitle eyebrow="Enkratno plačilo · brez naročnine" title="Preprosti paketi za vsak dogodek." text="Začnite brezplačno. Plačate šele, ko potrebujete več prostora in funkcij." /></div><div className="mt-14 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{PLANS.map(([name,price,tag,features]) => <article key={name} className={`relative flex min-h-[480px] flex-col rounded-[30px] p-7 ${name==="Plus" ? "bg-black text-white ring-4 ring-[#F4B400] shadow-2xl" : "border border-black/10 bg-white shadow-sm"}`}>{name==="Plus" && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#F4B400] px-4 py-1.5 text-[11px] font-black uppercase text-black">Najbolj priljubljen</span>}<p className="text-sm font-black uppercase tracking-[.15em]">{name === "Premium" ? weddingCopy("sl").plan : name}</p><p className="mt-2 text-sm opacity-50">{tag}</p><p className="mt-6 text-5xl font-black">{price}</p><ul className="mt-8 flex flex-1 flex-col gap-3 text-sm opacity-75">{[...features, ...(name === "Premium" ? [weddingCopy("sl").schedule, weddingCopy("sl").menu, weddingCopy("sl").songs, weddingCopy("sl").bingo] : [])].map(f => <li key={f}>✓ {f}</li>)}</ul><Link href={`/dashboard/new?plan=${name.toLowerCase()}&lang=sl`} className={`mt-8 rounded-full px-5 py-3.5 text-center font-black ${name==="Plus" ? "bg-[#F4B400] text-black" : "bg-black text-white"}`}>Izberi {name === "Premium" ? weddingCopy("sl").plan : name}</Link></article>)}</div></div></section>
