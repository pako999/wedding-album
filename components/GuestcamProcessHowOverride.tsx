...
const steps = [
  {
    ...
    src: "/how/guestcam-step-1-event.webp?v=4",
  ...
  },
  {
    ...
    src: "/how/guestcam-step-2-details.webp?v=4",
  ...
  },
  {
    ...
    src: "/how/guestcam-step-3-qr.webp?v=4",
      ...
      <div className="mt-12 grid gap-6 md:mt-16 md:grid-cols-3 md:gap-9">
        {steps.map((step, index) => (
          <div key={step.no} className="relative min-w-0">
            ...
            {index < steps.length - 1 && <div className="absolute -right-[28px] top-1/2 z-10 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border-4 border-[#F7F5F1] bg-[#F4B400] text-lg font-bold text-white md:flex" aria-hidden="true">→</div>}
          </div>
        ))}
      </div>
      <div className="mt-10 text-center sm:mt-12">
        <a href="/dashboard/new" className="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl bg-[#171A20] px-7 text-sm font-bold text-white transition-transform hover:-translate-y-0.5 sm:text-base">Ustvari svojo galerijo zdaj <span aria-hidden="true">→</span></a>
      </div>
    </div>,
    target,
  );
